...
export interface DataModalDescriptions<T extends Record<string, any>>
...
export function DataModal<T extends Record<string, any>>(
  ...
  function renderBody() {
    ...
    return (
      <div className="flex max-h-[80vh] flex-col gap-3 space-x-2 overflow-auto">
        {children}
      </div>
    )
  }
  ...
  return (
    <DataModalContext.Provider value={{ type }}>
      <Dialog {...rest}>
        <DialogContent className="sm:max-w- top-[20%] translate-y-[-20%] border-gray-200 md:max-w-xl lg:max-w-3xl dark:border-gray-800">
          <DialogHeader>
...
export function DataModalCreateOrUpdate({
  children,
}: {
  children: React.ReactNode
}) {
