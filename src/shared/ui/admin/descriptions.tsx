...
type ResponsiveNumber =
  | number
  | Partial<Record<keyof typeof tailwindBreakpoints, number>>
...
export function Descriptions({
  ...
  return (
    <div className="flex w-full flex-col space-y-2" {...props}>
      ...
      <div
        className={cn(
          bordered && 'rounded-sm border border-gray-200 dark:border-gray-800'
        )}
      >
        <Table className={cn('w-full border-collapse border-hidden')}>
          <TableBody>
            {blocks.map((block, rowIndex) => {
              const usedSum = block.reduce(
                (acc, it) => acc + getSpanValue(it.span, realColumn - acc),
                0
              )
                      ...
                      ? [
                          <TableHead
                            ...
                            className={cn(
                              ...
                              'text-foreground rounded-sm border border-gray-200 bg-gray-50 dark:border-gray-800 dark:bg-gray-900'
                            )}
                          >
                            {it.label}
                          </TableHead>,
  ...
  React.useEffect(() => {
    ...
    function handleResize() {
      ...
      for (const bp of ['sm', 'md', 'lg', 'xl', '2xl'] as const) {
        ...
        if (px && w >= px && column[bp as keyof typeof column]) {
          newVal = column[bp as keyof typeof column]!
        }
      }
      setVal(newVal)
    }
